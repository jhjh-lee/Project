...
public class ProducWriteService implements Service {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String pDes = mRequest.getParameter("pDes");
			String pType = "BEER"; /*  �ӽù��� mRequest.getParameter("pType");*/
			int pPrice = Integer.parseInt(mRequest.getParameter("pPrice"));
			int pStock = Integer.parseInt(mRequest.getParameter("pStock"));
			ProductDao pDao = ProductDao.getInstance();
			int result = pDao.productInsert(pCode, pName,pDes, pType, pFilename, pPrice, pStock);
			System.out.println();
			if(result == ProductDao.SUCCESS) {
				
				request.setAttribute("productInsert", "�긽��?��벑濡�? �꽦?��");
			}else {
				request.setAttribute("erroMsg", "�긽��?��벑濡�? �떎�뙣");
			}
		...
		// �뾽濡쒕뱶��? �뙆�씪�쓣 �냼�뒪�뤃�뜑濡� 蹂듭�?
				...
				if(!pFilename.equals("NOIMG.JPG") && serverFile.exists()) {
					...
					try {
						...
						os = new FileOutputStream("C:\\Users\\TJ\\Desktop\\jh3\\WebContent\\QNAFileboardUp\\"+pFilename);
